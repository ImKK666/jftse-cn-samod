...
public class TcpConnection {
    ...
    private int badPacketLimit = 15;
    private HashMap<Connection, Integer> badPacketSuspectMap = new HashMap<>();
    ...
    public Packet readPacket(Connection connection) throws IOException {
        ...
        while (true) {
            ...
            if (!this.isValidChecksum(data)) {
                // 读取异常数据包次数超过限制时断开连接
                if (isBadPacketOverTheLimit(connection)) {
                    log.info(String.format("连续收到 %s 的无效数据包已达到 %i 次，断开连接",
                            connection.getRemoteAddressTCP().getAddress().getHostAddress(),
                            badPacketLimit));
                    connection.close();
                } else {
                    log.debug(connection.getRemoteAddressTCP().getAddress().getHostAddress() + " 数据包无效");
                    try {
                ...
                if (badPacketSuspectMap.containsKey(connection))
                    badPacketSuspectMap.remove(connection);
            }
    ...
    private boolean isBadPacketOverTheLimit(Connection connection) {
        if (badPacketSuspectMap.containsKey(connection)) {
            int times = badPacketSuspectMap.get(connection) + 1;
            if (times < badPacketLimit) {
                badPacketSuspectMap.replace(connection, times);
            } else {
                badPacketSuspectMap.remove(connection);
                return true;
            }
        } else {
            badPacketSuspectMap.put(connection, 1);
        }
        return false;
    }
}
