...
public class GamePacketHandler {
    ...
    public void handleShopBuyRequestPacket(Connection connection, Packet packet) {
        ...
        int mint = player.getAccount().getAp();
...
//                .sum();
        ...
        for (Map.Entry<Product, Byte> data : productList.entrySet()) {
            ...
            switch (option) {
                case 0:
                    costs = +product.getPrice0();
                    break;
                case 1:
                    costs = +product.getPrice1();
                    break;
                case 2:
                    costs = +product.getPrice2();
                    break;
                default:
                    break;
            }
            switch (product.getPriceType().toUpperCase()) {
                case "GOLD":
                    goldCosts += costs;
                    break;
                case "MINT":
        ...
        log.debug("PlayerGold/Mint=" + gold + "/" + mint + " Costs=" + goldCosts + "/" + mintCosts);
        gold -= goldCosts;
        mint -= mintCosts;
        ...
        if (gold >= 0 && mint >= 0) {
            for (Map.Entry<Product, Byte> data : productList.entrySet()) {
                ...
                if (product.getCategory().equals(EItemCategory.PET_CHAR.getName())) {
                    //haveEnoughMoney += product.getPrice0();
                    continue;
                }
            ...
            playerService.setMint(player, mint);

            player = playerService.setMoney(player, gold);
            ...
            S2CShopBuyPacket shopBuyPacketAnswer;
            if(gold<0)
                shopBuyPacketAnswer = new S2CShopBuyPacket(S2CShopBuyPacket.NEED_MORE_GOLD, null);
            else if(mint<0)
                shopBuyPacketAnswer=new S2CShopBuyPacket(S2CShopBuyPacket.NEED_MORE_CASH, null);
            else
                shopBuyPacketAnswer=new S2CShopBuyPacket(S2CShopBuyPacket.GET_CASHINFO_FAILED, null);
            connection.sendTCP(shopBuyPacketAnswer);
        }
    }
    ...
    public void handleRoomCreateQuickRequestPacket(Connection connection, Packet packet) {
        ...
        Room room = new Room();
        room.setRoomId(this.getRoomId());
        room.setRoomName(String.format("%s 的房间", player.getName()));
        ...
        room.setUnk1((byte) 0);
        room.setSkillFree(true);   //默认允许球拍技能
        room.setQuickSlot(true);   //默认允许快捷栏
